...
public class Experiment {
	 ...
	 * @param totalUtil total taskset utilization of one VM
	 * @param total_vms total number of VMs
	 */
	public void RandomComponents(boolean newExp, double totalUtil, int total_vms)
	{
		...
		for(int i=0;i<total_vms;i++)
		{
			...
			comp.setPi(Pi);
			int Theta = comp.getMPR().getTheta();
			//comp.FFDecompose();
			comp.getTaskSet().setIndex(i);
			//Logging
			String util = Double.toString(myTasks.getUtilization());
			...
			String cntTasks = Integer.toString(myTasks.getSize());
			String currentPi = Integer.toString(Pi);
			String currentTheta = Integer.toString(Theta);
